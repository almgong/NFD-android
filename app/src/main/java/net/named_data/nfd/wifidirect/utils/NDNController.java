...
import net.named_data.nfd.wifidirect.runnable.FaceConsistencyRunnable;
...
import net.named_data.nfd.wifidirect.task.FaceCreateTask;
import net.named_data.nfd.wifidirect.task.FaceDestroyTask;
import net.named_data.nfd.wifidirect.task.RegisterPrefixTask;
import net.named_data.nfd.wifidirect.task.RibRegisterPrefixTask;
...
public class NDNController {
    ...
    private RegisterPrefixTask localhopProbeTask = null;    // reference to the task handling "/localhop/wifidirect"
    ...
    // TODO Perhaps consulting ARP table could resolve this (linking MAC to IP)?
    private HashMap<String, Peer> connectedPeers = null;            // { deviceAddress(MAC) : PeerInstance, ... }, contains MAC and device name
    private HashMap<String, Peer> peersMap = null;                  // { peerIp : PeerInstance }, contains at least Face id info
    ...
    public void registerOwnLocalhop() {
        if (!hasRegisteredOwnLocalhop) {
            ...
            localhopProbeTask = (RegisterPrefixTask) registerPrefix(mFace, PROBE_PREFIX + "/" +
                    IPAddress.getLocalIPAddress(), new OnInterestCallback() {
                @Override
                public void onInterest(Name prefix, Interest interest, Face face, long interestFilterId, InterestFilter filter) {
                    (new ProbeOnInterest()).doJob(prefix, interest, face, interestFilterId, filter);
                }
            }, false, 100);  // process event every 100 ms
        }
    }
    ...
    public void unregisterOwnLocalhop() {
        if (localhopProbeTask != null) {
            Log.d(TAG, "Unregistering own /localhop prefix...");
            localhopProbeTask.setStopProcessing(true);
            localhopProbeTask = null;
        }
    }
    ...
    /**
     * Attempts to P2P connect to a peer denoted by its MAC address.
     * @param peerDevice the WifiP2pDevice instance representing the
     *                   peer to connect to.
     */
    public void connectToPeer(final WifiP2pDevice peerDevice) {

        Runnable connectToPeerRunnable = new Runnable() {
            @Override
            public void run() {
                // config
                ...
                wifiP2pManager.connect(channel, config, new WifiP2pManager.ActionListener() {
                    ...
                    public void onSuccess() {
                        ...
                        Log.d(TAG, "Initiating connect() successful for: " + config.deviceAddress);
                    }
                    ...
                    public void onFailure(int reason) {
                        ...
                        Log.e(TAG, "There was an issue with initiating connect() for: " +
                                peerDevice.deviceName + ", reason: " + reasonString);

                        // notify the queue immediately (so that it can try to connect to the next
                        // peer if any)
                        connectQueue.notifyConnectAttemptComplete();
                    }
                });
            }
        };

        // queue up this connect() runnable
        connectQueue.enq(new ConnectQueueItem(connectToPeerRunnable, peerDevice.deviceName,
                peerDevice.deviceAddress));
        connectQueue.notifyQueueChanged();
    }

    /**
     * Stops/Pauses the connect queue so that no more connect()s are issued. This should be done
     * once the current device belongs to a group. Triggered in notifyConnectToPeerSuccess() which
     * in turn is called in WDBroadcastReceiver.
     */
    public void stopConnectQueue() {
        // it's a private class, so direct memeber access OK
        Log.d(TAG, "Stopped the connect queue.");
        connectQueue.readyForNextConnect = false;

        // clear queued peers (if they are still available later, they would be
        // re-added to the queue anyways)
        for (ConnectQueueItem item : connectQueue.queue) {
            // they have yet to be connected to, so just remove state
            connectedPeers.remove(item.deviceAddress);
        }

        connectQueue.queue.clear();
    }

    /**
     * Opposite of stopConnectQueue, this method will resume/begin the connect queue,
     * issuing connect() requests as appropriate. Note that in general, this should only
     * be called after stopConnectQueue() is. See usage in startDiscoveringPeers().
     */
    public void startConnectQueue() {
        Log.d(TAG, "Started the connect queue.");
        connectQueue.readyForNextConnect = true;
    }
    ...
    /* Internal queue implementation used in queueing connect() calls */
    private class ConnectQueue {
        private ConcurrentLinkedQueue<ConnectQueueItem> queue;  // in case of multi-thread access
        private boolean readyForNextConnect;                    // ready to call connect() again

        public ConnectQueue() {
            queue = new ConcurrentLinkedQueue<>();
            readyForNextConnect = true;
        }

        // queue up a new runnable for the next connect()
        public void enq(ConnectQueueItem item) {
            queue.add(item);
        }

        // notifies the queue that it should check for another potential peer to connect() to
        public void notifyQueueChanged() {
            // peek for next connect
            ConnectQueueItem next = queue.peek();

            // if object exists and queue is not currently waiting on another connect(), schedule
            // the next connect()
            if (next == null) {
                Log.d(TAG, "[ConnectQueue] No more queued peers to connect to.");
            } else if (readyForNextConnect) {

                // begin scheduling next connect()
                readyForNextConnect = false;
                next = queue.poll();
                Runnable connectToPeerRunnable = next.runnable;

                // precaution as to not overwhelm the WD framework with connect requests (especially
                // right when discoverPeers() is called)
                // add a random delay before connecting to the specified device
                // note: the delay spans from 2-7 seconds
                long randomDelay = new Double(Math.random() * 5000).intValue() + 2000;
                Log.d(TAG, "[ConnectQueue] Connecting to peer: " + next.deviceName + " in " +
                        randomDelay + "ms...");
                scheduledThreadPoolExecutor.schedule(connectToPeerRunnable, randomDelay,
                        TimeUnit.MILLISECONDS);
            } else {
                // if logic reaches here, means that there was a queued peer, but the
                // queue is currently waiting on another connect() call
                Log.d(TAG, "[ConnectQueue] Still waiting on another connect() or " +
                        "device is already in a group, " +
                        "have yet to issue connect for: "
                        + next.deviceName + ". Total queue size: " + queue.size());
            }
        }

        // notifies the queue of terminated connect() call. Note that this method should
        // be called in presence of both success and failure.
        public void notifyConnectAttemptComplete() {
            readyForNextConnect = true; // allow for the next ConnectQueueItem to be processed
            notifyQueueChanged();       // optimistically see if there are more peers in queue to connect to
        }
    }

    /* Convenience class for associating connect runnables with their device names/addr */
    class ConnectQueueItem {
        public Runnable runnable;
        public String deviceName;
        public String deviceAddress;

        public ConnectQueueItem(Runnable runnable, String deviceName, String deviceAddress) {
